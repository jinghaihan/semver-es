...
import { validVersions } from '../fixtures/valid-versions'
...
test('return SemVer arg to ctor if options match', (t) => {
  ...
  t.equal(new SemVer(s, { loose: true, includePrerelease: true }), s, 'get same object when options match')
...
test('invalid increments', (t) => {
  t.throws(
    ...
    new Error('invalid increment argument: identifier is empty'),
  )
  t.throws(
    () => new SemVer('1.2.3-dev').inc('prerelease', 'dev', false),
    new Error('invalid increment argument: identifier already exists'),
  )
  t.throws(
    () => new SemVer('1.2.3').inc('prerelease', 'invalid/preid'),
    new Error('invalid identifier: invalid/preid'),
  )
