...
import { comparatorIntersection } from '../fixtures/comparator-intersection'
...
test('intersect comparators', (t) => {
  ...
  comparatorIntersection.forEach(([c0, c1, expect, includePrerelease]) =>
    t.test(`${c0} ${c1} ${expect}`, (t) => {
      ...
      t.equal(comp0.intersects(comp1, { includePrerelease }), expect, `${c0} intersects ${c1}`)

      t.equal(comp1.intersects(comp0, { includePrerelease }), expect, `${c1} intersects ${c0}`)
      t.end()
    }))
})
...
test('ANY matches anything', (t) => {
  ...
  t.ok(c1.test(Comparator.ANY), 'anything matches ANY')
  t.end()
})

test('invalid comparator parse throws', (t) => {
  t.throws(() => new Comparator('foo bar baz'), new TypeError('Invalid comparator: foo bar baz'))
  t.end()
})
